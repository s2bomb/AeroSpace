...
By default, a new window becomes the next sibling of the most recently used window in its container (`append`).

Alternatively, every new window can split the most recently used window's cell perpendicular to its container (`split-mru`):

[source,toml]
----
default-new-window-placement = 'split-mru'
----

Consecutive windows then form the "dwindle"/"spiral" shape known from other tiling window managers:
...
If the most recently used window is the only window in its container, the new window is appended as a plain sibling — the container itself already represents the split.
Accordion containers always append.
